...
void PutChar( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
    ...
    GetASCIICode(buffer,ASCI);//得到ASCI对应的16位数组保存在buffer中
    for( i=0; i<16; i++ )
    {
        tmp_char = buffer[i];
        for( j=0; j<8; j++ )
...
void PutNum( uint16_t Xpos, uint16_t Ypos, uint8_t num, uint16_t NumColor, uint16_t bkColor )
{
	u8 tmp_num,i,j;
	num=num+16;
	for( i=0; i<16; i++ )
    {
        ...
        for( j=0; j<8; j++ )
        {
            if( (tmp_num >> 7 - j) & 0x01 == 0x01 )
            {
                LCD_SetPoint( Xpos + j, Ypos + i, NumColor );
            }
            else
            {
                LCD_SetPoint( Xpos + j, Ypos + i, bkColor );
...
void GUI_Num(u16 x, u16 y, u32 num,u8 len,u16 Color, u16 bkColor)
{
u8 i;  u8 b[10];
	for(i=0;i<len;i++)
	{ b[len-i-1]=num%10;
...
void LCD_ShowPicture(u16 x, u16 y, u16 wide, u16 high,u8 *pic)
{
  ...
  num = wide * high*2 ;
	...
	{  
		...
		temp = temp << 8;
		temp = temp | pic[tmp];
		LCD_SetPoint(Xp,Yp,temp);//逐点显示
		tmp += 2;
		if (Xp == x+wide-1){Xp=x;Yp=Yp+1;}
		else {Xp=Xp+1;}
	}
	while(tmp < num);	
}
